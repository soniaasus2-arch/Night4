-- DAVI HUB UNIFICADO (Noites 2 e 3) - Detecção por objetos
local key = "DAVI2024"
...
function carregarHubUnificado()
    -- Detecta a noite pela presença de objetos específicos
    local isNight3 = false
    local isNight2 = false
    
    if workspace:FindFirstChild("AmmoPiles") then
        isNight3 = true
        print("🔍 Noite 3 detectada (AmmoPiles presente)")
    end
    if workspace:FindFirstChild("Stalker") then
        isNight2 = true
        print("🔍 Noite 2 detectada (Stalker presente)")
    end
    
    if isNight3 then
        carregarNoite3()
    elseif isNight2 then
        carregarNoite2()
    else
        -- Fallback: tenta encontrar pelas pastas "Windows" (Noite 2) ou "Lodge" (Noite 3)
        if workspace:FindFirstChild("Windows") then
            print("🔍 Fallback: Windows encontrado -> Noite 2")
            carregarNoite2()
        elseif workspace:FindFirstChild("Lodge") then
            print("🔍 Fallback: Lodge encontrado -> Noite 3")
            carregarNoite3()
        else
            print("⚠️ Não foi possível detectar a noite. Carregando Noite 2 por padrão.")
            carregarNoite2()
        end
    end
end

function carregarNoite2()
    ...
    local function refreshNPC()
        ...
        if mutant and not mutant:FindFirstChild("ESP_NPC") then addNPCESP(mutant, Color3.fromRGB(255, 0, 0)) end
        local stalker = workspace:FindFirstChild("Stalker")
        if stalker and not stalker:FindFirstChild("ESP_NPC") then addNPCESP(stalker, Color3.fromRGB(255, 100, 0)) end
    ...
    print("✅ DAVI HUB - Noite 2 carregado")
end

function carregarNoite3()
    local plr = game.Players.LocalPlayer
    local gui = Instance.new("ScreenGui")
    gui.Parent = plr:WaitForChild("PlayerGui")
    local f = Instance.new("Frame")
    ...
    -- Teleports Noite 3
    btn("🏕️ Cabana 1", function() plr.Character.HumanoidRootPart.CFrame = CFrame.new(99.8, 4.5, -247.2) end)
    btn("🏕️ Cabana 2", function() plr.Character.HumanoidRootPart.CFrame = CFrame.new(-36.9, 4.5, 68.7) end)
    btn("🏕️ Cabana 3
